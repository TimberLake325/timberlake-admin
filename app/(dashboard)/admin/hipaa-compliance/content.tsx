...
import { saveHipaa } from '@/services/hipaaService';
...
import HipaaCompliancePageData from './pageData';
...
const HipaaContent = ({ initialData }: { initialData: HipaaDataState | null }) => {
    const [saving, setSaving] = useState(false);
    const [isConfirmOpen, setIsConfirmOpen] = useState(false);
    const { showToast } = useToast();

    const [data, setData] = useState<HipaaDataState>(initialData || {
        metadata: {},
        pageData: {
            pageTitle: '',
            pageSubTitle: '',
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await saveHipaa(data as unknown as Record<string, unknown>);
            if (response.success) {
    ...
    return (
        <div className="p-3 lg:p-6 bg-[#fcfcfc] min-h-screen pb-32">
            <ConfirmationModal
                ...
                title="Persist HIPAA Configuration"
                message="Are you sure you want to save these changes to the HIPAA Compliance page? This will overwrite the current live settings."
            ...
            <PageHeader
                description="Manage HIPAA compliance information and security protocols"
