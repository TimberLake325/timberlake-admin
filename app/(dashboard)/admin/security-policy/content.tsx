...
import { saveSecurity } from '@/services/securityService';
...
import SecurityPolicyPageData from './pageData';
...
const SecurityContent = ({ initialData }: { initialData: SecurityDataState | null }) => {
    const [saving, setSaving] = useState(false);
    const [isConfirmOpen, setIsConfirmOpen] = useState(false);
    const { showToast } = useToast();

    const [data, setData] = useState<SecurityDataState>(initialData || {
        metadata: {},
        pageData: {
            pageTitle: '',
            pageSubTitle: '',
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await saveSecurity(data as unknown as Record<string, unknown>);
            if (response.success) {
    ...
    return (
        <div className="p-3 lg:p-6 bg-[#fcfcfc] min-h-screen pb-32">
            <ConfirmationModal
                ...
                title="Persist Security Configuration"
                message="Are you sure you want to save these changes to the Security Policy page? This will overwrite the current live settings."
            ...
            <PageHeader
                description="Manage technical security measures and organizational safeguards"
