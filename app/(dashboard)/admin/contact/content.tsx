...
import { saveContactPage } from '@/services/contactPageService';
import { FiSave, FiSearch, FiInfo, FiActivity } from 'react-icons/fi';
...
interface ContactPageDataState {
    metadata?: any;
    pageData?: any;
}

const ContactPageContent = ({ initialData }: { initialData: ContactPageDataState | null }) => {
    const [saving, setSaving] = useState(false);
    const [isConfirmOpen, setIsConfirmOpen] = useState(false);
    const { showToast } = useToast();

    const [data, setData] = useState<ContactPageDataState>(initialData || {
        metadata: {},
        pageData: {}
    });
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await saveContactPage(data as Record<string, unknown>);
            if (response.success) {
    ...
    return (
        <div className="p-3 lg:p-6 bg-[#fcfcfc] min-h-screen pb-32">
            <ConfirmationModal
                ...
                title="Persist Contact Configuration"
                message="Are you sure you want to save these changes to the Contact Page? This will overwrite the current live settings."
            ...
            <PageHeader
                description="Manage contact page information, form fields, and communication settings"
                onSave={() => setIsConfirmOpen(true)}
                saving={saving}
                pageClick={() => window.location.href = '/admin/contact/submissions'}
                pageClickText="Submissions"
            />
