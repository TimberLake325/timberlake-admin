...
import { saveTerms } from '@/services/termsService';
import { FiSave, FiSearch, FiFileText, FiActivity } from 'react-icons/fi';
...
import TermsPageData from './pageData';
...
const TermsContent = ({ initialData }: { initialData: TermsDataState | null }) => {
    const [saving, setSaving] = useState(false);
    const [isConfirmOpen, setIsConfirmOpen] = useState(false);
    const { showToast } = useToast();

    const [data, setData] = useState<TermsDataState>(initialData || {
        metadata: {},
        pageData: {
            pageTitle: '',
            pageSubTitle: '',
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await saveTerms(data as unknown as Record<string, unknown>);
            if (response.success) {
    ...
    return (
        <div className="p-3 lg:p-6 bg-[#fcfcfc] min-h-screen pb-32">
            <ConfirmationModal
                ...
                title="Persist Terms Configuration"
                message="Are you sure you want to save these changes to the Terms & Conditions? This will overwrite the current live settings."
            ...
            <PageHeader
                description="Manage terms of service, usage agreements, and legal requirements"
