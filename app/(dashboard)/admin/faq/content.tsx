...
import { saveFAQ } from '@/services/faqService';
import { FiSearch, FiHelpCircle, FiActivity } from 'react-icons/fi';
...
import FaqPageData from './pageData';
...
const FaqPageContent = ({ initialData }: { initialData: FAQDataState | null }) => {
    const [saving, setSaving] = useState(false);
    const [isConfirmOpen, setIsConfirmOpen] = useState(false);
    const { showToast } = useToast();

    const [data, setData] = useState<FAQDataState>(initialData || {
        metadata: {},
        faqs: []
    });
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await saveFAQ(data as unknown as Record<string, unknown>);
            if (response.success) {
    ...
    return (
        <div className="p-3 lg:p-6 bg-[#fcfcfc] min-h-screen pb-32">
            <ConfirmationModal
                ...
                title="Persist FAQ Configuration"
                message="Are you sure you want to save these changes to the FAQ? This will overwrite the current live settings."
            ...
            <PageHeader
                description="Manage FAQ content and SEO metadata for the help center"
